...
class Container:
    ...
    def __exit__(self, exc_type, exc_val, exc_tb) -> None:
        ...
        self.container.stop()
        time.sleep(5)  # Wait for the container to stop before potentially starting a new one
...
def test_concurrent_requests() -> None:
    with Container(workers=8) as (container, cookies):
        ...
        p = Process(target=add_item, args=(generate_items(1)[0], cookies,))
        p.start()
        p.join()
        ...
        amount_of_items = 200
        old_amount = get_items_len(cookies)
        processes = [Process(target=add_item, args=(item, cookies,)) for item in generate_items(amount_of_items)]
