function _addToTable(user) {
  ...
  let kommentar = "";
  if (is_banned) {
    status = `Bannlyst siden ${new Date(
      user.last_banned_at
    ).toLocaleDateString()}`;
...
function updateEditUserModal(user) {
   ...
   */
  document.getElementById("edit-user-name").value = decodeURIComponent(
    user.name
  );
  document.getElementById("edit-user-classroom").value = decodeURIComponent(
    user.classroom
  );
  document.getElementById("edit-user-personal-email").value =
    decodeURIComponent(user.personal_email) === "null"
      ? ""
      : decodeURIComponent(user.personal_email);
  document.getElementById("classroom_teacher_disabled").value =
    decodeURIComponent(user.classroom_teacher);
  document.getElementById("classroom_teacher").value = decodeURIComponent(
    user.classroom_teacher
  );
  document.getElementById("edit-user-banned").checked =
    decodeURIComponent(user.banned) === "1";
  document.getElementById("edit-user-banned-reason").value = decodeURIComponent(
    user.banned_reason
  );
...
async function saveEditChanges() {
  ...
  let personal_email = document.getElementById(
    "edit-user-personal-email"
  ).value;
